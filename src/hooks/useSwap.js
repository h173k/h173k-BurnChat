...
export function useSwap(connection, wallet) {
   ...
   * Inverts the constant-product formula. Adds `bufferPct` on top so slippage
  ...
  const quoteSOLForH173K = useCallback(async (targetH173k, bufferPct = 4) => {
    ...
    let outRaw = BigInt(Math.ceil(targetH173k * Math.pow(10, TOKEN_DECIMALS)))
    if (outRaw <= 0n) return 0
    if (outRaw >= reserveH173K) throw new Error('Amount exceeds pool liquidity')

    // amountInAfterFee = outRaw * reserveIn / (reserveOut - outRaw)
    const amountInAfterFee = (outRaw * reserveSOL) / (reserveH173K - outRaw) + 1n
    // undo the 0.25% fee taken from input: amountIn = afterFee * 10000 / 9975
    const amountInLamports = (amountInAfterFee * 10000n) / 9975n + 1n
